...
async function initNotifState() {
  ...
  if (!("serviceWorker" in navigator) || !("PushManager" in window)) {
    sel.disabled = true;
    var lbl = sel.querySelector(".notif-label");
    if (lbl) lbl.textContent = "unsupported";
    return;
  }
  try {
...
var _notifLabels = {off: "turn on notifications", mention: "mentions", all: "all"};
function updateNotifStyle() {
  ...
  if (lbl) lbl.textContent = _notifLabels[_prevMode] || _notifLabels.off;
  var dd = document.getElementById("notif-dropdown");
  if (dd) dd.setAttribute("data-active", _prevMode);
}
var _prevMode = "off";
...
function initResize(handle) {
  handle.addEventListener('pointerdown', function(evt) {
    ...
    var sy = evt.clientY;
    var sh = ta.offsetHeight;
    function mv(e) {
      ta.style.height = Math.max(40, sh - (e.clientY - sy)) + 'px';
    }
    function up() {
